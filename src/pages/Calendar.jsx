...
export default function Calendar() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.08fr_0.42fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white p-5 shadow-xl shadow-slate-950/10">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-slate-950">Week view</h3>
            ...
            <div className="mt-5 overflow-x-auto">
              <div className="min-w-[900px]">
                <div className="grid grid-cols-7 gap-2.5">
                  {weekDays.map((day) => (
                    <div key={day.key} className="rounded-2xl bg-slate-50 p-2.5 text-center">
                      <p className="text-sm font-semibold text-slate-900">{day.num}</p>
                ...
                <div className="mt-3 grid grid-cols-7 gap-2.5">
                  {weekDays.map((day) => (
                    <div
                      key={day.key}
                      className="min-h-[250px] rounded-[1.25rem] border border-slate-200 bg-slate-50/70 p-2.5"
                    >
                      <div className="space-y-2.5">
                        {(tasksByDay[day.key] ?? []).length === 0 ? (
                          <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 px-3 py-3 text-center text-xs text-slate-400">
                            No tasks
                          </div>
                        ) : (
          ...
          {unscheduledTasks.length > 0 ? (
            <div className="mt-6 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
              <h4 className="text-sm font-semibold uppercase tracking-[0.25em] text-slate-500">Unscheduled tasks</h4>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-950">Upcoming due dates</h3>
            ...
            <div className="mt-5 space-y-3">
              {tasks.filter((task) => task.due_date).slice(0, 6).length === 0 ? (
                ...
                tasks
                  ...
                  .slice(0, 6)
                  .map((task) => (
                    <div key={task.id} className="rounded-2xl border border-slate-200 px-4 py-3">
                      <p className="font-medium text-slate-950">{task.title}</p>
